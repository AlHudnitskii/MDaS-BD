...
def admin_logs_view(request):
    ...
    logs = LogRepository.get_filtered(
        action_type=action_type or None,
        status=status or None,
        user_filter=user_filter or None,
        days=int(days) if days.isdigit() else 7,
    )
    stats = LogRepository.get_statistics()
...
def cleanup_logs_view(request):
    ...
    try:
        LogRepository.cleanup_old(days)
        LogRepository.log_action(
            user_id, 'LOGS_CLEANUP', {'days': days}, 'SUCCESS'
        )
        messages.success(request, f'Logs older than {days} days cleaned up.')
